...
class ImageUploader:
    def __init__(self, root: Any):
        self.root: Any = root
        self.root.title("Connie's Uploader 0.9.7")
        ...
        # Gallery Options for pixhost.to
        self.pix_gallery_frame: ttk.Labelframe = ttk.Labelframe(self.pixhost_tab, text="Gallery Options", padding=15)
        self.pix_gallery_frame.grid(row=1, column=0, columnspan=2, pady=15, sticky="ew")
        self.pix_gallery_frame.columnconfigure(1, weight=1)

        self.pix_gallery_var: tk.BooleanVar = tk.BooleanVar(value=False)
        self.pix_gallery_check: ttk.Checkbutton = ttk.Checkbutton(self.pix_gallery_frame, text="Create Gallery", variable=self.pix_gallery_var, command=self.toggle_gallery_entry)
        self.pix_gallery_check.grid(row=0, column=0, columnspan=2, pady=10, sticky="w")

        self.pix_gallery_name_label: ttk.Label = ttk.Label(self.pix_gallery_frame, text="Gallery Name:")
        self.pix_gallery_name_label.grid(row=1, column=0, padx=(0, 10), pady=10, sticky="w")
        self.pix_gallery_name_entry: ttk.Entry = ttk.Entry(self.pix_gallery_frame, width=30)
        self.pix_gallery_name_entry.grid(row=1, column=1, pady=10, sticky="ew")

        self.pix_gallery_name_label.grid_remove()
        self.pix_gallery_name_entry.grid_remove()
        ...
        self.make_entry_context_menu(self.pix_gallery_name_entry)
        ...
        self.gallery_hash: Optional[str] = None
        self.gallery_upload_hash: Optional[str] = None
        ...
        self.create_tooltip(self.pix_gallery_check, "Enable to create a named gallery for the uploads")
    ...
    def toggle_gallery_entry(self) -> None:
        if self.pix_gallery_var.get():
            self.pix_gallery_name_label.grid()
            self.pix_gallery_name_entry.grid()
        else:
            self.pix_gallery_name_label.grid_remove()
            self.pix_gallery_name_entry.grid_remove()
    ...
    def load_settings(self) -> None:
        ...
        if os.path.exists("settings.json"):
            try:
                ...
                self.pix_gallery_var.set(pix.get("create_gallery", False))
    ...
    def save_settings(self) -> None:
        ...
        settings: Dict[str, Any] = {
            ...
            "pixhost.to": {
                ...
                "create_gallery": self.pix_gallery_var.get(),
            },
            "general": {
    ...
    def start_upload(self) -> None:
        ...
        if service == "pixhost.to" and self.pix_gallery_var.get():
            gallery_name = self.pix_gallery_name_entry.get().strip()
            if not gallery_name:
                messagebox.showerror("Error", "Gallery name is required when creating a gallery.")
                return
            try:
                url = Constants.PIX_GALLERIES_URL
                headers = {
                ...
                self.gallery_hash = json_data.get("gallery_hash")
                self.gallery_upload_hash = json_data.get("gallery_upload_hash")
                self.gallery_url = json_data.get("gallery_url")
                if not self.gallery_hash or not self.gallery_upload_hash or not self.gallery_url:
                    raise ValueError("Missing gallery information in response.")
                self.log_message(f"Created gallery: {self.gallery_url}")
            except Exception as e:
    ...
    def upload_worker(self, service: str, username: Optional[str], password: Optional[str], content_var: Optional[tk.StringVar], 
                      ...
                      imx_cover_var: tk.BooleanVar, pix_cover_var: tk.BooleanVar,
                      gallery_hash: Optional[str], gallery_upload_hash: Optional[str]) -> None:
    ...
    def check_threads(self) -> None:
        ...
        if not self.active_threads and self.queue.empty():
            ...
            # Finalize gallery if applicable
